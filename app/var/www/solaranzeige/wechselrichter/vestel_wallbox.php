...
Log::write( "-------------   Start  vestel_wallbox.php   --------------------- ", "|--", 6 );
...
if (file_exists( "/var/www/pipe/".$GeraeteNummer.".befehl.steuerung" )) {
  ...
  $rc = fwrite( $COM1, $sendenachricht );
  $Antwort = bin2hex( fread( $COM1, 1000 )); // 1000 Bytes lesen
  Log::write( "Stromänderung fix: ".$Antwort, "   ", 3 );
...
do {
  ...
  ***************************************************************************/
  ...
  $rc = ModBus::modbus_register_lesen( $COM1, "0100", "0019", "String3", $WR_ID, "04" );
  $aktuelleDaten["Seriennummer"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0130", "0032", "String3", $WR_ID, "04" );
  $aktuelleDaten["Ladepunkt_ID"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0190", "000A", "String3", $WR_ID, "04" );
  $aktuelleDaten["Ladepunkt_Brand"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0210", "0005", "String3", $WR_ID, "04" );
  $aktuelleDaten["Ladepunkt_Modell"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0230", "0032", "String3", $WR_ID, "04" );
  $aktuelleDaten["Firmware"] = trim( $rc["Wert"] );
  Log::write( "Firmware Version: ".$aktuelleDaten["Firmware"], "   ", 6 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0400", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["MaxPower"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0404", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["AnzPhasen"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1000", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Status"] = trim( $rc["Wert"] ); // Stationsstatus
  $rc = ModBus::modbus_register_lesen( $COM1, "1001", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["LadungAktiv"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1002", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["LadestationEin"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1004", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Kabelstatus"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1006", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["ErrorCode"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1008", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Strom_R"] = trim( $rc["Wert"] ) / 1000;
  $rc = ModBus::modbus_register_lesen( $COM1, "1010", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Strom_S"] = trim( $rc["Wert"] ) / 1000;
  $rc = ModBus::modbus_register_lesen( $COM1, "1012", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Strom_T"] = trim( $rc["Wert"] ) / 1000;
  $rc = ModBus::modbus_register_lesen( $COM1, "1014", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Spannung_R"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1016", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Spannung_S"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1018", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Spannung_T"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1020", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Leistung"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1024", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Leistung_R"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1028", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Leistung_S"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1032", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Leistung_T"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1036", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Zaehlerstand"] = ($rc["Wert"] * 100);
  $rc = ModBus::modbus_register_lesen( $COM1, "1100", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["MaxLadestrom"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1102", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["MinLadestrom"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1104", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["HardwareLimit"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1106", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["KabelLimit"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "1502", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["LadeLeistung"] = trim( $rc["Wert"] );
  // Zeit   151104 = 15:11:04 Uhr)
  $rc = ModBus::modbus_register_lesen( $COM1, "1504", "0002", "HEX", $WR_ID, "04" );
  $aktuelleDaten["StartLadung"] = hexdec( trim( $rc["Wert"] ));
  // Ergebnis in Minuten
  $rc = ModBus::modbus_register_lesen( $COM1, "1508", "0002", "U32", $WR_ID, "04" );
  $aktuelleDaten["Ladezeit"] = floor( $rc["Wert"] / 60 );
  // Zeit   151104 = 15:11:04 Uhr)
  $rc = ModBus::modbus_register_lesen( $COM1, "1512", "0002", "HEX", $WR_ID, "04" );
  $aktuelleDaten["EndeLadung"] = hexdec( trim( $rc["Wert"] ));
  $rc = ModBus::modbus_register_lesen( $COM1, "2002", "0001", "U16-1", $WR_ID, "03" );
  $aktuelleDaten["Timeout"] = trim( $rc["Wert"] );
  //  Holding Register  R/W  Function 03
  //  Stromvorgabe
  $rc = ModBus::modbus_register_lesen( $COM1, "5004", "0001", "U16-1", $WR_ID, "03" );
  $aktuelleDaten["Ladestrom"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "6000", "0001", "U16-1", $WR_ID, "03" );
  $aktuelleDaten["Alive"] = trim( $rc["Wert"] );
  // Log::write( "rc. ".print_r($rc,1), "   ", 7 );
  ...
  ****************************************************************************/
  if (file_exists($basedir."/custom/vestel_wallbox_math.php" )) {
    include $basedir.'/custom/vestel_wallbox_math.php'; // Falls etwas neu berechnet werden muss.
  }
...
Ausgang:
//
Log::write( "-------------   Stop   vestel_wallbox.php   --------------------- ", "|--", 6 );
return;
?>
