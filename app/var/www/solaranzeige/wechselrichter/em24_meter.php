...
Log::write( "------------------   Start  em24_meter.php    -------------------------- ", "|--", 6 );
...
do {
  ...
  //  UnitID 03 = default
  ****************************************************************************/
  $rc = ModBus::modbus_register_lesen( $COM1, "0000", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_R"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0002", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_S"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0004", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_T"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0006", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_RS"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0008", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_ST"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0010", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Spannung_RT"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0012", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Strom_R"] = round( $rc["Wert"] / 1000, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0014", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Strom_S"] = round( $rc["Wert"] / 1000, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0016", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Strom_T"] = round( $rc["Wert"] / 1000, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0018", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Leistung_R"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0020", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Leistung_S"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0022", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Leistung_T"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0051", "0001", "U16-1", $WR_ID, "04" );
  $aktuelleDaten["Frequenz"] = round( $rc["Wert"] / 10, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0040", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["AC_Leistung"] = round( $rc["Wert"] / 10, 3 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0049", "0001", "S16", $WR_ID, "04" );
  $aktuelleDaten["PF_Leistung"] = round( $rc["Wert"] / 1000, 3 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0046", "0001", "S16", $WR_ID, "04" );
  $aktuelleDaten["PF_R"] = round( $rc["Wert"] / 1000, 3 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0047", "0001", "S16", $WR_ID, "04" );
  $aktuelleDaten["PF_S"] = round( $rc["Wert"] / 1000, 3 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0048", "0001", "S16", $WR_ID, "04" );
  $aktuelleDaten["PF_T"] = round( $rc["Wert"] / 1000, 3 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0211", "0002", "U32", $WR_ID, "04" );
  ...
  $rc = ModBus::modbus_register_lesen( $COM1, "0060", "0002", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["Wh_Bezug"] = round( $rc["Wert"] * 1000, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0078", "0001", "S32-1", $WR_ID, "04" );
  $aktuelleDaten["Wh_Einspeisung"] = round( $rc["Wert"] * 1000, 2 );
  $rc = ModBus::modbus_register_lesen( $COM1, "0961", "0004", "F64", $WR_ID, "04" );
  $aktuelleDaten["GesamterLeistungsbedarf"] = ($aktuelleDaten["Wh_Bezug"] + $aktuelleDaten["Wh_Einspeisung"]); // Dummy
  $rc = ModBus::modbus_register_lesen( $COM1, "0011", "0001", "Hex", $WR_ID, "04" );
  $aktuelleDaten["HerstellerID"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "20480", "0007", "", $WR_ID, "04" );
  $aktuelleDaten["Seriennummer"] = Utils::Hex2String( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0770", "0001", "", $WR_ID, "04" );
  ...
  ****************************************************************************/
  if (file_exists($basedir."/custom/em24_meter_math.php" )) {
    include $basedir.'/custom/em24_meter_math.php'; // Falls etwas neu berechnet werden muss.
  }
...
if (file_exists( $StatusFile )) {
  ...
  ***************************************************************************/
  if (date( "H:i" ) == "00:00" or date( "H:i" ) == "00:01") {
    $Tagesdaten = array("BezugGesamtHeute" => 0, "EinspeisungGesamtHeute" => 0);
    $rc = file_put_contents( $StatusFile, serialize( $Tagesdaten ));
    Log::write( "BezugGesamtHeute  gesetzt.", "o- ", 5 );
  }
...
Ausgang:fclose( $COM1 );
Log::write( "------------------   Stop   em24_meter.php    -------------------------- ", "|--", 6 );
return;
?>
