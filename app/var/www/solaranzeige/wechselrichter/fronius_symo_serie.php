...
//
*****************************************************************************/
$StatusFile = $Pfad."/database/".$GeraeteNummer.".WhProTag_fronius.txt";
...
do {
  ...
  ****************************************************************************/
  ...
  if ($rc["APIVersion"] != 1) {
    $funktionen->log_schreiben( "Ist die API freigeschaltet? Falsche API Version,", "   ", 3 );
    $funktionen->log_schreiben( "oder keine Verbindung zum Wechselrichter.".print_r( $rc, 1 ), "   ", 3 );
    $funktionen->log_schreiben( "Exit.... ", "XX ", 3 );
    exit;
  }
  switch ($rc["CompatibilityRange"]) {
    ...
    case "1.8-1":
    case "1.6-3":
      $funktionen->log_schreiben( "API Version: ".$rc["APIVersion"]." CompatibilityRange: ".$rc["CompatibilityRange"], "   ", 2 );
      break;

    default:
      $funktionen->log_schreiben( "API Version: ".$rc["APIVersion"]." CompatibilityRange noch unbekannt: ".$rc["CompatibilityRange"], "   ", 2 );
      break;
  }
  ...
  if ($rc["CompatibilityRange"] == "1.6-3" or $rc["CompatibilityRange"] == "1.5-18" or $rc["CompatibilityRange"] == "1.5-16" or $rc["CompatibilityRange"] == "1.5-13" or $rc["CompatibilityRange"] == "1.8-1") {
    ...
    if (isset($JSON_Daten["Head"]["Status"]["Code"]) and $JSON_Daten["Head"]["Status"]["Code"] == 0 and !empty($JSON_Daten["Body"]["Data"])) {  // Neu 17.3.2023
      ...
      if ($WR_Adresse != $Teile[1]) {
        $funktionen->log_schreiben( "Wechselrichter Adresse ist nicht stimmig: ".print_r($Teile,1), "   ", 5 );
      }
