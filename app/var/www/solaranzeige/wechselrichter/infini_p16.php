...
Log::write("-----------   Start  infini_p16.php   ------------------------ ","|--",6);
...
do {
  ...
  if ($Wert === false) {
    Log::write("Datenübertragung vom Wechselrichter war erfolglos! [Status]","    ",9);
    $rc = "";
  }
  ...
  if ($Wert === false) {
    Log::write("Datenübertragung vom Wechselrichter war erfolglos! [Modell]","    ",9);
    $rc = "";
  }

  Log::write(substr($Antwort,1,45)."  i: ".$k,"    ",10);
  ...
  if ($Wert === false) {
    Log::write("Datenübertragung vom Wechselrichter war erfolglos! [Gesamte Energie]","    ",9);
    $rc = "";
  }
  else {
    Log::write(substr($Antwort,1,8)."  i: ".$k,"    ",10);
  ...
  fputs ($USB, "\0");
  $rc = fgets($USB,4096); // Alte Daten löschen
  $CRC = Utils::onebytechecksum("QEY".date("Y"));
  ...
  Log::write("Gesamt Energie für ".date("Y").": ".substr($Antwort,1,8)."  i: ".$k,"    ",10);
  ...
  Log::write(substr($Antwort,1,132)."  i: ".$k,"    ",8);
  ...
  if ($Wert === true and strlen($Antwort) > 132) {
    ...
    **************************************************************************/
    if ( file_exists($basedir."/custom/infini_p16_math.php")) {
      include $basedir.'/custom/infini_p16_math.php';  // Falls etwas neu berechnet werden muss.
    }
...
Log::write("-----------   Stop   infini_p16.php    ----------------------- ","|--",6);
