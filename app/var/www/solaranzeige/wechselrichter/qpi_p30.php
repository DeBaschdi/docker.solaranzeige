...
do {
  ...
  for ($k = 1; $k < 200; $k++) {
    ...
    $rc = "";
    Log::write( $Antwort, "    ", 10 );
    if (substr( trim($Antwort,"\0"), - 1 ) == "\r" and substr( $Antwort, 0, 1 ) == "(") {
      Log::write( "Antwort: ".bin2hex( $rc ), "    ", 9 );
      if (substr( $Antwort, 1, 3 ) == "NAK") {
        $Wert = false;
        Log::write( "Info: Wechselrichter Antwortet auf 'QPI' mit mit NAK! (ist kein Fehler) ", "    ", 5 );
      }
      else {
        $aktuelleDaten["Protokoll"] = substr( $Antwort, 3, 2 );
        Log::write( "Protokoll: ".substr( $Antwort, 3, 2 ), "    ", 8 );
        $Wert = true;
      }
      $rc = "";
      break;
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Modus] ".$Antwort, "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  if ($Wert === true) {
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Warnungen] ".$Antwort, "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  if ($Wert === true) {
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Seriennummer] ".$Antwort, "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  ...
  //fputs( $USB, "\0" );
  $rc = fgets( $USB, 4096 ); // Alte Daten löschen
  $CRC = Utils::hex2str( dechex( Utils::CRC16Normal( "QVFW" )));
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Seriennummer]", "    ", 5 );
    $rc = "";
  }
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Standardwerte] ".$Antwort, "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  Log::write( substr( $Antwort, 1, - 3 )."  i: ".$k, "    ", 8 );
  ...
  if (substr( $Antwort, 1, 3 ) == "NAK") {
    Log::write( "QPIGS2: ".substr( $Antwort, 1, 3 ), "    ", 8 );
  }
  else {
    Log::write("QPIGS2: ". substr( $Antwort, 1, - 3 )."  i: ".$k, "    ", 10 );
  ...
  //fputs( $USB, "\0" );
  $rc = fgets( $USB, 4096 ); // Alte Daten löschen
  $CRC = Utils::hex2str( dechex( Utils::CRC16Normal( "QMN" )));
  fputs( $USB, "QMN".$CRC."\r" );
  usleep( $Timebase ); //  [normal 10000] Es dauert etwas, bis die ersten Daten kommen ...
  for ($k = 1; $k < 200; $k++) {
    $rc = fgets( $USB, 4096 ); // 4096
  ...
  if (substr( $Antwort, 1, 3 ) == "NAK") {
    Log::write( "QMN: ".substr( $Antwort, 1, 3 ), "    ", 8 );
  }
  else {
    Log::write( "Modell: ".substr( trim($Antwort,"\0"), 1, - 3 ), "    ", 6 );
  ...
  if ($Wert === false) {
    Log::write( "Datenübertragung vom Wechselrichter war erfolglos! [Daten]", "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  // echo $Antwort;
  Log::write( trim(substr( $Antwort, 1, 105 ))."  i: ".$k, "    ", 7 );
  ...
  Log::write( "Firmware:  ".$aktuelleDaten["Firmware"], "   ", 1 );
  ...
  **************************************************************************/
  if (file_exists($basedir."/custom/qpi_p30_math.php" )) {
    include $basedir.'/custom/qpi_p30_math.php'; // Falls etwas neu berechnet werden muss.
  }
