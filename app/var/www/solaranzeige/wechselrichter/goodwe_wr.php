...
Log::write("----------------------   Start  goodwe_wr.php   --------------------- ", "|--", 6);
...
do {
  ...
  ****************************************************************************/
  ...
  if ($rc["ok"] == 0) {
    Log::write("Der Wechselrichter sendet kleine Daten.", "   ", 5);
    goto Ausgang;
  }
  Log::write("DeviceID: (HEX) ".$WR_ID, "   ", 5);
  $Befehl["RegisterAddress"] = "0210";
  $Befehl["RegisterCount"] = "0005";
  $rc = Phocos::phocos_pv18_auslesen($USB1, $Befehl);
  $aktuelleDaten["Modell"] = Utils::hex2string($rc["data"]);
  Log::write("Modell: ".$aktuelleDaten["Modell"], "   ", 5);
  if (strtoupper(substr($aktuelleDaten["Modell"], - 2)) == "XS") {
    ...
    $Befehl["RegisterCount"] = "0002";
    $rc = Phocos::phocos_pv18_auslesen($USB1, $Befehl);
    $aktuelleDaten["VPV"] = Utils::hex2string($rc["data"]);
...
Ausgang:Log::write("----------------------   Stop   goodwe_wr.php   --------------------- ", "|--", 6);
return;
?>