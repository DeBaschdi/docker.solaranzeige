...
Log::write( "-------------   Start  wallbe_wallbox.php   --------------------- ", "|--", 6 );
...
do {
  ...
  ***************************************************************************/
  ...
  $rc = ModBus::modbus_register_lesen( $COM1, "0100", "0001", "String2", "FF", "04" );
  $aktuelleDaten["Status"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0102", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Ladezeit"] = trim( $rc["Wert"] );
  $rc = ModBus::modbus_register_lesen( $COM1, "0105", "0002", "String2", "FF", "04" );
  $aktuelleDaten["Firmware"] = trim( $rc["Wert"] );
  if (strlen( $aktuelleDaten["Firmware"] ) >= 4) {
    $aktuelleDaten["Firmware"] = substr( $aktuelleDaten["Firmware"], 1, 1 ).".".substr( $aktuelleDaten["Firmware"], 2, 2 );
  }
  $rc = ModBus::modbus_register_lesen( $COM1, "0107", "0001", "Dec16Bit", "FF", "04" );
  $aktuelleDaten["ErrorCode"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0108", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Spannung_R"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0110", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Spannung_S"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0112", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Spannung_T"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0114", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Strom_R"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0116", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Strom_S"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0118", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Strom_T"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0120", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Leistung"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0128", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["GesamtLeistung"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0130", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["MaxLeistung"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0132", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["LadeLeistung"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0134", "0002", "Dec32Bit", "FF", "04" );
  $aktuelleDaten["Frequenz"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0204", "0001", "Dec16Bit", "FF", "02" );
  $aktuelleDaten["LadebedingungenOK"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0300", "0001", "Dec16Bit", "FF", "03" );
  $aktuelleDaten["Ladestrom"] = ($rc["Wert"] / 10);
  $rc = ModBus::modbus_register_lesen( $COM1, "0525", "0001", "Dec16Bit", "FF", "03" );
  $aktuelleDaten["Locktime"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0526", "0001", "Dec16Bit", "FF", "03" );
  $aktuelleDaten["Unlocktime"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0528", "0001", "Dec16Bit", "FF", "03" );
  $aktuelleDaten["MaxLadestrom"] = ($rc["Wert"] / 10);
  $rc = ModBus::modbus_register_lesen( $COM1, "0400", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["LadungAktiv"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0402", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["LadestationEin"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0403", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["PerModbusAktiviert"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0405", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["StatusRegister1steuern"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0406", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["StatusRegister2steuern"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0407", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["StatusRegister3steuern"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0408", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["StatusRegister4steuern"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0436", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Ladung_erlaubt"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0439", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Kabel_angeschlossen"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0440", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Kabel_entriegeln"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0461", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["AufOKwarten"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0465", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Kabel_verriegelt"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0467", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Ladung_unterbrochen"] = $rc["Wert"];
  $rc = ModBus::modbus_register_lesen( $COM1, "0468", "0001", "Dec16Bit", "FF", "01" );
  $aktuelleDaten["Ladung_unterbrechen"] = $rc["Wert"];

  if ($aktuelleDaten["Firmware"] == "0.95") {
    Log::write( "Wallbe ECO 2.0 mit FW == 0.95 ", " ", 7 );
    $aktuelleDaten["Ladung_erlaubt"] = 0;
    $aktuelleDaten["Kabel_angeschlossen"] = 0;
    $aktuelleDaten["Kabel_entriegeln"] = 0;
  }
  ...
  ***************************************************************************/
  ...
  /**************************************************************************
  //  Alternative Berechnung, wenn kein Stromzähler angeschlossen ist.
  ***************************************************************************/
  if (file_exists($basedir."/custom/wallbe_oz.php" )) {
    include ($basedir."/custom/wallbe_oz.php");
  }

  /**************************************************************************
  //  Ende Wallbox Zusatz wenn kein Zähler angeschlossen ist
  ***************************************************************************/
  $FehlermeldungText = "";
  ...
  ****************************************************************************/
  if (file_exists($basedir."/custom/wallbe_wallbox_math.php" )) {
    include $basedir.'/custom/wallbe_wallbox_math.php'; // Falls etwas neu berechnet werden muss.
  }
...
Ausgang:Log::write( "-------------   Stop   wallbe_wallbox.php   --------------------- ", "|--", 6 );
return;
?>
